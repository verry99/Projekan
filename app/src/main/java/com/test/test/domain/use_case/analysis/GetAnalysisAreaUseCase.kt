...
class GetAnalysisAreaUseCase @Inject constructor(
    private val analysisRepository: AnalysisRepository
) {
    suspend operator fun invoke(token: String): Flow<Resource<AnalysisGetAreaResponse>> = flow {
        emit(Resource.Loading())
        try {
            val data = analysisRepository.getAnalysisArea(token)
